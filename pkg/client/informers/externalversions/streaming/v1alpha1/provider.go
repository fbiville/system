...
import (
	...
	v1alpha1 "github.com/projectriff/system/pkg/client/listers/streaming/v1alpha1"
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.StreamingV1alpha1().Providers(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.StreamingV1alpha1().Providers(namespace).Watch(options)
			},
		},
		&streaming_v1alpha1.Provider{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *providerInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&streaming_v1alpha1.Provider{}, f.defaultInformer)
}
